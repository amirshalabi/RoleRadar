...
tests/         598 tests, offline (fake Postgres/Qdrant/OpenAI clients)
.github/       CI workflow (test.yml) + Dependabot (dependabot.yml) — runs the suite on push/PR, no secrets needed
```
...
[`.github/dependabot.yml`](.github/dependabot.yml) opens a weekly PR for outdated `pip` dependencies (`requirements.txt`) and GitHub Actions versions, each run through the same CI workflow above before merge.
